...
class SyncWorker(QObject):
    ...
    status = Signal(str)  # status: new/modified/same
...
class ScanWorker(QObject):
    ...
    progress = Signal(int, int, str)
    finished = Signal(list, str)  # entries, status_text
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        commit_top = QHBoxLayout()
        self.refresh_commits_btn = QPushButton("刷新 Commit 列表")
        self.refresh_commits_btn.clicked.connect(self._refresh_commits)
        commit_top.addWidget(self.refresh_commits_btn)
        commit_top.addStretch()
        commit_layout.addLayout(commit_top)

        self.commit_list = QPlainTextEdit()
        self.commit_list.setReadOnly(True)
        self.commit_list.setMaximumHeight(100)
        self.commit_list.setPlaceholderText("点击「刷新 Commit 列表」查看工作区提交记录")
        commit_layout.addWidget(self.commit_list)

        msg_label = QLabel("Commit Message:")
        commit_layout.addWidget(msg_label)

        self.commit_msg = QTextEdit()
        self.commit_msg.setPlaceholderText("在此编辑正式 commit message...")
        ...
        self.term_btn = QPushButton("🖥 打开终端模式")
    ...
    def _on_scan_finished(self, entries: list[FileEntry], summary: str):
        ...
        self.sync_btn.setEnabled(len(entries) > 0)
        self._log(summary)
    ...
    def _refresh_commits(self):
        ...
        if commits:
            ...
            for i, c in enumerate(commits, 1):
                ...
                lines.append(f"  {i}. [{c.type}{scope}] {c.subject}")
            self.commit_list.setPlainText("\n".join(lines))
            self._log(f"发现 {len(commits)} 个新 commit")

            # 自动生成模板
            template = build_commit_template(commits, self.config)
            self.commit_msg.setPlainText(template)
        else:
            self.commit_list.setPlainText("无新 commit")
            self.commit_msg.setPlainText("[ANBM-N] chore: sync update\n\n# 未检测到新 commit，请手动编辑\n")
    ...
    def _start_sync(self):
        ...
        if not selected:
            QMessageBox.warning(self, "提示", "没有选中任何文件")
            return
...
def _first_run_dialog() -> str | None:
    """首次运行：弹出引导对话框让用户选择备份目录（有父窗口确保置前）"""
