...
def gitgo_export(project: str, minimal: bool = False,
...
@mcp.tool(
    description="手动触发工具记忆快照（.claude/.codex/.codebuddy），保存到 backup 的 .gitgo/memories/。sync 时自动执行。"
)
def gitgo_memory_snapshot(project: str) -> dict:
    ...
    result = snapshot_tool_memories(
        session.workspace_path, session.backup_path, proj,
    )
    return {"snapped": result.get("snapped", []), "timestamp": result.get("timestamp", "")}


@mcp.tool(
    description="从 backup 恢复工具记忆到 workspace。默认使用最新快照，可通过 ts 指定时间戳。"
)
def gitgo_memory_restore(project: str, ts: str | None = None) -> dict:
    ...
    if not session.backup_path:
        return {"error": "NO_BACKUP_CONFIGURED"}
    return restore_tool_memories(
        session.backup_path, session.workspace_path, snapshot_timestamp=ts,
    )


@mcp.tool(
    description="列出 backup 中所有可用的工具记忆快照。"
)
