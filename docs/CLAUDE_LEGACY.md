...
## ⚠️ 已知事故（2026-05-29，gitgo dogfooding 期间）

### 事故：`git reset --hard` 摧毁 Lexi 工作区未提交代码

**操作**：在 Lexi workspace 执行 `git reset --hard HEAD~3` 清理狗粮测试 commit。

**后果**：Lexi CC 在最后 commit 之后、reset 之前写的所有代码（`reciter_tab.py` 等 10+ 文件）因为没有 commit，被 `--hard` 用旧版本覆盖。git 历史完好但未提交改动全部丢失，无法恢复。

**根因**：清理测试残留时用了 `--hard` 而非 `--soft`。`--soft` 只移 HEAD 指针不动 working tree，不会删除 CC 的未提交代码。

**教训**：
- **在 gitgo 管理的项目工作区里，绝对禁止 `git reset --hard`**
- 清理测试 commit 用 `git revert`（保留历史）或 `git reset --soft`（不动文件）
- 如果必须重置 working tree，先在 `git stash` 保存未提交改动
- Policy Engine 应该在未来版本中检测 `git reset --hard` 并阻止执行（v0.28 Part C）
